...
function sharerbox(options) {
	...
	<div id="sharerbox-share-icon-wrap">
		...
		<div id="sharerbox-share-text" style="cursor: pointer" class="text-sm">Share</div>
	</div>
</section>`;
	...
	var shareText = document.getElementById('sharerbox-share-text');
	...
	var currentUrl = encodeURIComponent('https://fastly.picsum.photos/id/958/200/200.jpg?hmac=WdLUMERHKTLw-sP-eIf1-JlwdIT2ZY12zf4JbnQR_s8');
	...
	// Setting URLs for supported social-media sharing links
		...
		var facebookURL = `https://www.facebook.com/sharer/sharer.php?u={{image}}&alt={{url}}`;

		// Twitter sharer hyperlink
		var tweetURL = `https://twitter.com/intent/tweet?text={{image}}&url={{url}}`;

		// Whatsapp sharer hyperlink
		var whatsappURL = `whatsapp://send?text={{image}}%0D%0A{{url}}`;

		// Reddit sharer hyperlink
		var redditURL = `https://www.reddit.com/submit/?url={{image}}%0D%0A{{url}}`;

		// LinkedIn sharer hyperlink
		var linkedinURL = `https://www.linkedin.com/sharing/share-offsite/?url={{image}}%0D%0A{{url}}`;

		// Pinterest sharer hyperlink
		var pinterestURL = `https://www.pinterest.com/pin/create/button/?url=`;
		...
		//Email
		var sendEmailURL = `mailto:?subject="Chat Transcript from {{url}}"&body={{image}}`;

		// Pop-up window opener function
		function openWindow(url) {
			Toastify({
				text: url ? "Sharing your content ..." : "Preparing clipboard to copy...",
				duration: 3000,
				newWindow: true,
				close: true,
			...
			html2canvas(div,{ useCORS: true }).then(
				async function (canvas) {
					...
					try {
						...
						if(url) {
							let link = url.replace("{{image}}", imageUrl)
							link = link.replace("{{url}}", "https://bratai.com")
							window.open(link, '_blank', `width=${screen.width / 2}px, height=${screen.height / 2}px`);
						}else {
							copyURL(imageUrl)
						}
					}
	...
	shareText.addEventListener('click', flipIcon);
